...
void stear(int stearing) {
	...
	float curve;
	float tmp_forwardSpeed;
	// motorRightRev += forwardSpeed;
	// motorLeftRev += forwardSpeed;

	// curve = (stearing_float*stearing_float*stearing_float)/600.0;
	curve = stearing_float / 20.0;

	// motorRightRev += curve/curveSpeed;
	// motorLeftRev += -curve/curveSpeed;
	tmp_forwardSpeed = forwardSpeed*(1-abs(curve));
	tmp_forwardSpeed<2 ? tmp_forwardSpeed = 2 : false;

	motorRightRev += tmp_forwardSpeed*(1+curve);
	motorLeftRev += tmp_forwardSpeed*(1-curve);
...
void TSL1401_setup() {
	...
	//  adc->printError();
	TSL1401_Init(500, 5000ul); // 2000, 20000ul
}
...
void TSL1401_ReadLine() {
	...
	for(int i=0;i<130;i++) {
		...
		val8<30 ? Tx1_buf[i] = 0 : Tx1_buf[i] = 254;
